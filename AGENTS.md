...
3. `sync/lint.go` runs `internal/lint` over the resolved compose config and records a `lint` phase on the deploy timeline. Advisory only — findings never abort the reconcile; policy enforcement stays in the renderer (step 4), which validates *after* applying render overrides.
...
- **Path containment** (`internal/safepath.ResolveContained`): the `Validate*` helpers only inspect the path *string* — they reject `..`, absolute paths and bad extensions, which is everything a request parameter can express, but they cannot see what the filesystem does with the result. Git preserves symlinks on checkout (mode 120000), so a repository shipping `docker-compose.yml -> /etc/passwd` passes every string check: the extension check constrains the *link's name*, never its target. `ResolveContained` resolves symlinks (including for a path whose trailing segments do not exist yet) and verifies the result stays under a root. Use it before **any** read whose path is influenced by repository content or request input — `compose.ResolveFile`/`ReadFile`/`Config` all do, via `ConfigOptions.Root`, which defaults to `WorkDir` so a forgotten root still cannot escape its own directory.
