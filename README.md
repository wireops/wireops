...
`SECRET_KEY` and `BOOTSTRAP_TOKEN` are required — see [Quick Start](#quick-start) above. For the full server/worker env var reference, SMTP, OIDC/SSO, secret providers (Vault/Infisical), and SOPS+age, see the [Environment Variables wiki page](https://github.com/wireops/wireops/wiki/Environment-Variables).
...
Full plugin interface, config fields, and label/compose examples: [Integration Plugin System wiki page](https://github.com/wireops/wireops/wiki/Integration-Plugin-System).
