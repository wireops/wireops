...
// ExtractProjectName reads the top-level `name` field from a rendered
// compose file. The renderer (see sync.Renderer.GenerateRevision) always
// bakes this in as the wireops stack's own sanitized name, so callers that
// need the exact project name `docker compose` is using for this stack
// (e.g. post-deploy status checks) should read it from here rather than
// recomputing it from a work directory path, which can drift out of sync
// with what was actually rendered.
func ExtractProjectName(data []byte) (string, error) {
	var doc struct {
		Name string `yaml:"name"`
	}
	if err := yaml.Unmarshal(data, &doc); err != nil {
		return "", fmt.Errorf("failed to parse compose name: %w", err)
	}
	if doc.Name == "" {
		return "", fmt.Errorf("compose file has no top-level 'name' field")
	}
	return doc.Name, nil
}

// RewriteAutoNamedResources fixes up network/volume definitions whose "name"
// field was auto-generated by `docker compose config` from the old project
// name (e.g. the implicit default network becomes "<oldName>_default").
// `docker compose config` bakes these concrete resource names in at
// config-resolution time, before the renderer gets a chance to override the
