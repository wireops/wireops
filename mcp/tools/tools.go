...
func Register(server *mcp.Server, c *client.Client) {
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		Description: "Generate a valid standalone wireops.yaml file from structured fields, ready to commit to a repository. This is the legacy two-file layout — for a new stack, prefer scaffold_stack, which by default embeds an equivalent 'x-wireops' block directly in the compose file instead (no separate wireops.yaml needed). Use this tool directly only to add/edit a standalone wireops.yaml for a repository that already uses the two-file layout. Does not create or modify any stack — the caller commits the returned content itself.",
	}, generateWireopsYAML())
