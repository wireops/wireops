...
<template>
  <div :class="['border rounded-lg p-3 space-y-2', props.class, {
    ...
    <div class="flex items-start gap-2">
      <UIcon :name="getErrorIcon(parsed.type)" class="w-5 h-5 shrink-0 mt-0.5" :class="{
      ...
      <div class="flex-1 min-w-0">
        ...
        <p v-if="parsed.suggestion" class="text-xs mt-1" :class="{
          'text-red-700 dark:text-red-300': parsed.type !== 'network',
          'text-orange-700 dark:text-orange-300': parsed.type === 'network'
        }">
          💡 {{ parsed.suggestion }}
        </p>
        <a v-if="parsed.docLink" :href="parsed.docLink" target="_blank" rel="noopener noreferrer" class="text-xs underline mt-1 inline-block" :class="{
    ...
    <details v-if="parsed.original" :open="showDetails" class="text-xs">
      ...
      <pre class="mt-2 p-2 rounded text-xs overflow-x-auto whitespace-pre-wrap" :class="{
